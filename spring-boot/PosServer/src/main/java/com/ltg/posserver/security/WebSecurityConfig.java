package com.ltg.posserver.security;
...
//		auth.inMemoryAuthentication().withUser("admin").password("password").roles("ADMIN");
        auth.jdbcAuthentication().dataSource(dataSource).usersByUsernameQuery(
                "select aa.usua_username as \"username\", \n"
                + "aa.usua_password as \"password\", \n"
                + "(case when aa.usua_estado='ACTIVO' then 1 else 0 end) as \"enabled\" \n"
                + "from segu_usuario aa \n"
                + "WHERE aa.usua_username=?")
                .authoritiesByUsernameQuery(
                        "select cc.usua_username AS \"username\", \n"
